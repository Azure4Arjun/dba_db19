...
CREATE FUNCTION f_dba19_sys_RunDurationStats 
(
...
BEGIN
	...
	SELECT @avg = avg(h.run_duration) , @min = min(h.run_duration), @max = max(h.run_duration) 
	...
	IF @run_item = 0 SET @Result = @avg;
	IF @run_item = 1 SET @Result = @min;
	IF @run_item = 2 SET @Result = @max;
...
GRANT EXECUTE ON [dbo].[f_dba19_sys_RunDurationStats] TO [db_proc_exec]
GO
