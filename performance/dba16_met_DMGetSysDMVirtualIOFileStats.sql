...
GO
-- ======================================================================================
-- dba19_met_DMGetSysDMVirtualIOFileStatsBO
--
--
-- Calls:		None
--
-- Description:	checks sys.dm_io_virtual_file_stats looking for average read
--				stalls (latency) over 200 milliseconds and average write stalls
--				over 100 milliseconds.
--
--				Reads over 15ms are a bottleneck
...
BEGIN
	...
	DECLARE @AvgReadStalls INT
	DECLARE @AvgWriteStalls INT

	SET @AvgReadStalls = 200;
	SET @AvgWriteStalls = 100;
	...
	WHERE a.num_of_reads > 0
		...
		AND (
			CAST(a.io_stall_read_ms / (1.0 * a.num_of_reads) AS INT) > @AvgReadStalls
			OR CAST(a.io_stall_write_ms / (1.0 * a.num_of_writes) AS INT) > @AvgWriteStalls
		)
