...
/****** Object:  Job [DBA SQL Patch Level Status Report]    Script Date: 04/06/2011 11:35:13 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [DBA Reporting]    Script Date: 04/06/2011 11:35:14 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA SQL Patch Level Status Report', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'SQL Server detailed patch level', 
		@category_name=N'DBA Reporting', 
		@owner_login_name=N'sa', 
		@notify_email_operator_name=N'dbservices', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Run the report]    Script Date: 04/06/2011 11:35:14 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run the report', 
		...
		@command=N'C:\Program Files (x86)\Microsoft SQL Server\100\DTS\Binn\DTExec.exe /FILE "C:\Program Files (x86)\Microsoft SQL Server\100\DTS\Packages\dbinfrastructurestatus\pkg_dba_checkpatchlevels.dtsx" /MAXCONCURRENT " -1 " /CHECKPOINTING OFF /REPORTING E', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [E-mail the report]    Script Date: 04/06/2011 11:35:15 ******/
...
SET @subj = '' SQL Patch Level Status report''
SET @attch = ''''
SET @Msg = ''**** SQL Patch Level Status reports can now be found on the N drive @ N:\203 - Information Technology Shared Data\databaseadministration\xls\patchlevel\''
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DBA SQL Patch Level Status Report', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=127, 
		@freq_subday_type=1, 
