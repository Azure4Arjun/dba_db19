...
BEGIN
	...
	--SET @headr2 = '-- '
	SET @headr7 = '-- Date			Modified By			Changes' + ' ' + @crlf
			...
			DECLARE hdr_cur CURSOR FOR
				SELECT p.name, p.create_date, p.modify_date, p.type,  --p.[object_id], p.[principal_id], p.[schema_id], p.[parent_object_id],
					...
					FROM sys.procedures p
						JOIN [sys].[parameters]  a ON (p.[object_id] = a.[object_id])
						JOIN [sys].[types]       t ON (a.[user_type_id] = t.[user_type_id])
						JOIN [sys].[syscomments] c ON (p.[object_id] = c.id)
					...
					BEGIN
						...
						IF @rowcnt = 1
							BEGIN
								...
								SET @headr8 = '-- ' + @crdate_txt     + '   Developer           Initial Coding.' + ' ' + @crlf
								SET @headr11 = '-- ' + @moddate_txt   + '   Developer           Additional Coding.' + ' ' + @crlf
								SET @headr12 = '-- ' + @curdate_txt   + '   Aron E. Tekulsky    Add headings.' + ' ' + @crlf
						...
						-- we have a new sp so print last one
							BEGIN
								SET @finalheader =  @finalheader + @headr4 + @headr10 + @headr5 + @headr10 + @headr6 + @headr2 + @crlf + @headr7 + @headr8 
