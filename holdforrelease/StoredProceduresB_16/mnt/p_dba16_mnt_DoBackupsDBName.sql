...
-- 05/04/2012	Aron E. Tekulsky	Add code for dbname for individual db.
-- 03/05/2018   Aron E. Tekulsky    Update to Version 140.
-- ======================================================================================
...
-- ======================================================================================
--
CREATE PROCEDURE p_dba16_mnt_DoBackupsDBName 
...
-- end ola code --------------------------------------
	...
	IF dba_db16.[dbo].[f_dba16_utl_GetSemicolon](@backup_type) > 0
		BEGIN
	...
	IF dba_db16.[dbo].[f_dba16_utl_GetSemicolon](@backup_location) > 0
		BEGIN
 ...
 --@database_type
	IF dba_db16.[dbo].[f_dba16_utl_GetSemicolon](@database_type) > 0
		BEGIN
				...
				BEGIN
					DECLARE db_cur CURSOR FOR
						...
						WHERE d.recovery_model in (1,2,3) and -- 1 full 2 bulk logged 3 simple
							...
							e.db_backup_db = 1 AND-- 0 no backup 1 yes backup
							d.name = @dbname;
				END
		...
		IF @verify_backup = 'Y'
			BEGIN
				SET @query_cmd2 = 'RESTORE VERIFYONLY FROM  DISK = N' + '''' + 
					@backup_location + @database_name + '_backup_' + 
	 				@backup_date_text + @backup_time_text + '.' + @filetype + '''' +
					' WITH  FILE = ' + convert(varchar(25),@backupSetId) + ',  NOUNLOAD,  NOREWIND'
...
END
GO
GRANT EXECUTE ON p_dba16_mnt_DoBackupsDBName TO [db_proc_exec] AS [dbo]
GO
