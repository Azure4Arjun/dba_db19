...
-- ======================================================================================
--
CREATE PROCEDURE p_dba16_mnt_GetFragmentation 
...
-- create table to hold db names
	...
	IF [dbo].[f_dba16_utl_GetAlwaysOnNumeric]() = 1 
		SET @alwayson = 1;
		...
		BEGIN
			...
			INSERT INTO #TEMP(name,dbstatus)
				...
				--AND name in ('iie_enterprise')--'iie_enterprise','iiedb1')
					...
					[dbo].[f_dba16_utl_GetDBRole](name) = 1
				order by name asc;
		END
	ELSE 
		BEGIN
...
END
GO
GRANT EXECUTE ON p_dba16_mnt_GetFragmentation TO [db_proc_exec] AS [dbo]
GO
