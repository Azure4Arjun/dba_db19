...
GO
-- ======================================================================================
-- p_dba19_met_GetCompleteDBSpecs
--
-- Arguments:	None
--				None
--
...
-- ======================================================================================
--
CREATE PROCEDURE p_dba19_met_GetCompleteDBSpecs 
...
-- first we get the basic database information - non system db only
	...
	ORDER BY dbname ASC, obj_name ASC;
	...
	IF (@@version LIKE ('%SQL Server 2005%'))
		BEGIN
			SELECT '*** DB SSIS Packages ***', f.foldername, p.name, p.description, p.createdate, l.loginname, p.packagetype, p.vermajor, p.verminor, p.verbuild
	...
	-- get server logins
	EXEC [dbo].[p_dba19_sec_GetLoginsWServerRoles];
		
		PRINT '*** DB Users ***';

	-- get all users of the database
	EXEC [dbo].[p_dba19_sec_GetAllUsersByDB];;
			
	PRINT '*** DB User Roles ***';

	--Get all roles for each user in each db
	EXEC [dbo].[p_dba19_sec_GetDBUserDBRolesByDB];
			
	PRINT '*** DB Securables ***';

	-- get all securabels for each user in each db
	EXEC [dbo].[p_dba19_sec_GetDBUserSecurablesByDB];
...
END
GO
GRANT EXECUTE ON p_dba19_met_GetCompleteDBSpecs TO [db_proc_exec] AS [dbo]
GO
