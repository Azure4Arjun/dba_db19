...
/****** Object:  StoredProcedure [dbo].[p_dba16_stl_StealthGetBackupJobsMaxRunDate]    Script Date: 8/17/2016 8:12:11 AM ******/
DROP PROCEDURE [dbo].[p_dba16_stl_StealthGetBackupJobsMaxRunDate]
GO
...
-- ==============================================================================
-- p_dba16_stl_StealthGetBackupJobsMaxRunDate
--
-- Arguments:	None
--				None
--
...
CREATE procedure [dbo].[p_dba16_stl_StealthGetBackupJobsMaxRunDate] 
...
AS
	...
	--	name			sysname)
			...
			SET @cmd = 'SELECT ' + '''' + @server_name + '''' + ',h.instance_id, h.job_id, h.step_id, h.step_name,
				...
				dba_db16.dbo.f_dba16_utl_ConvertIntTimeToString(h.run_time) as run_time, h.server,
			...
			FROM [' + @server_name + '].msdb.dbo.sysjobhistory h
				...
				JOIN [' + @server_name + '].dba_db16.dbo.v_dba16_sys_MaxBackupJobDate m ON (h.job_id = m.job_id)
		...
		PRINT @cmd
			...
			INSERT dba_db16.dbo.dbbackups (servern, instance_id, job_id, step_id, step_name,
