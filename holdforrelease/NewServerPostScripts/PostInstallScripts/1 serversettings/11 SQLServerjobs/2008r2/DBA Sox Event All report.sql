...
/****** Object:  Job [DBA Sox event all Report]    Script Date: 05/18/2012 16:20:25 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [DBA Reporting]    Script Date: 05/18/2012 16:20:25 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA Sox event all Report', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'List all of the database events on all servers.', 
		@category_name=N'DBA Reporting', 
		@owner_login_name=N'IIE\dbservices', 
		@notify_email_operator_name=N'aron', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Run the report]    Script Date: 05/18/2012 16:20:25 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run the report', 
		...
		@command=N'C:\Program Files (x86)\Microsoft SQL Server\90\DTS\Binn\DTExec.exe /FILE "C:\Program Files (x86)\Microsoft SQL Server\90\DTS\Packages\sox\pkg_dba_sox_allevents_report.dtsx" /MAXCONCURRENT " -1 " /CHECKPOINTING OFF /REPORTING E', 
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [e-mail the report]    Script Date: 05/18/2012 16:20:25 ******/
...
SET @subj = '' DBA Sox event all Report''
SET @attch = ''''
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DBA Sox event all Report', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=127, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'f596dd7e-654f-4db5-ab86-00bd38b29f39'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
