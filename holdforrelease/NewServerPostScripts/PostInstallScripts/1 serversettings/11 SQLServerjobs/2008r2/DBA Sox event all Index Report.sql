...
/****** Object:  Job [DBA Sox event all Index Report]    Script Date: 6/27/2012 4:27:55 PM ******/
EXEC msdb.dbo.sp_delete_job @job_id=N'f636261a-23b1-4a65-945c-d105d5919171', @delete_unused_schedule=1
GO
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [DBA Reporting]    Script Date: 6/27/2012 4:27:55 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA Sox event all Index Report', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'List all of the database index maintenance events on all servers.', 
		@category_name=N'DBA Reporting', 
		@owner_login_name=N'IIE\dbservices', 
		@notify_email_operator_name=N'aron', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Run the report]    Script Date: 6/27/2012 4:27:55 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run the report', 
		...
		@command=N'C:\Program Files (x86)\Microsoft SQL Server\90\DTS\Binn\DTExec.exe /FILE "C:\Program Files (x86)\Microsoft SQL Server\90\DTS\Packages\sox\pkg_dba_sox_indexevents_report.dtsx" /MAXCONCURRENT " -1 " /CHECKPOINTING OFF /REPORTING E', 
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [e-mail the report]    Script Date: 6/27/2012 4:27:55 PM ******/
...
SET @subj = '' DBA Sox event all Index Report''
SET @attch = ''''
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DBA Sox event all Index Report', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=127, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'c06039d6-d3eb-4877-b8ac-b31043b9e4fb'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
