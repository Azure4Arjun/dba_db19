...
/****** Object:  Job [DBA Error Step Report]    Script Date: 08/13/2010 15:46:53 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [DBA Reporting]    Script Date: 08/13/2010 15:46:53 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA Error Step Report', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Run the report]    Script Date: 08/13/2010 15:46:53 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run the report', 
		...
		@command=N'C:\Program Files (x86)\Microsoft SQL Server\100\DTS\Binn\DTExec.exe /FILE "C:\Program Files (x86)\Microsoft SQL Server\100\DTS\Packages\testemail\stepemailtest1.dtsx" /MAXCONCURRENT " -1 " /CHECKPOINTING OFF /REPORTING E', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [e-mail report]    Script Date: 08/13/2010 15:46:53 ******/
...
SET @subj = '' step error report''
SET @attch = ''''
SET @Msg = ''**** Error step reports can now be found on the N drive @ N:\203 - Information Technology Shared Data\databaseadministration\xls\failedjobs\''
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [copy file]    Script Date: 08/13/2010 15:46:54 ******/
...
--	  EXEC @Result = xp_cmdshell ''  Del  /q \\usnypdba01\ssis_errors\ams_connector\iiewide_contract_log.log''
	  EXEC @Result = xp_cmdshell ''  copy /y \\usnypdba01\ssis_errors\testemail\failed_job_steps_template.xls \\usnypdba01\ssis_errors\testemail\failed_job_steps.xls ''
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DBA error step report', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=127, 
		@freq_subday_type=1, 
