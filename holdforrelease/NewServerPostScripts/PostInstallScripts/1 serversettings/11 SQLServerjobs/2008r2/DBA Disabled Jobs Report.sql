...
/****** Object:  Job [DBA Disabled Jobs Report]    Script Date: 08/13/2010 15:46:37 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [DBA Reporting]    Script Date: 08/13/2010 15:46:37 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA Disabled Jobs Report', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Run the report]    Script Date: 08/13/2010 15:46:37 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run the report', 
		...
		@command=N'C:\Program Files (x86)\Microsoft SQL Server\100\DTS\Binn\DTExec.exe /FILE "C:\Program Files (x86)\Microsoft SQL Server\100\DTS\Packages\dbinfrastructurestatus\get_disabled_jobs.dtsx" /MAXCONCURRENT " -1 " /CHECKPOINTING OFF /REPORTING E', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [e-mail report]    Script Date: 08/13/2010 15:46:37 ******/
...
SET @subj = '' Disabled jobs report''
SET @attch = ''''
SET @Msg = ''****Disabled Jobs reports can now be found on the N drive @ N:\203 - Information Technology Shared Data\databaseadministration\xls\disabledjobs\''
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [copy files]    Script Date: 08/13/2010 15:46:37 ******/
...
/* Delete the files from the server */
	  EXEC @Result = xp_cmdshell ''  copy /y \\usnypdba01\ssis_errors\testemail\disabled_jobs_template.xls \\usnypdba01\ssis_errors\testemail\disabled_jobs.xls ''
  ...
  IF @Result = 1
     BEGIN
          ...
          SET @Msg = ''There are no disabled jobs template files to copy as of '' + @DateText + '' at '' + @TimeText
        RAISERROR (@Msg, 18, 1)
     END
', 
		@database_name=N'master', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DBA Disabled job report', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=127, 
		@freq_subday_type=1, 
