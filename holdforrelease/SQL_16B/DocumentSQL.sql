...
print ' F. See backup schedules ( full and transaction )                '
print '***************************************************************'
...
select convert(varchar(30),login_time,109) as 'Service start up ' from master..sysprocesses where spid = 1
...
LEFT OUTER JOIN	msdb..backupset b
	...
	AND b.backup_start_date = (SELECT MAX(backup_start_date)
					...
					WHERE database_name = b.database_name
						AND type = 'D')		-- full db backups only, not tl
WHERE	s.name <> 'tempdb'
