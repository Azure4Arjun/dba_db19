...
BEGIN
	...
	SELECT TOP 10
			s.wait_type , s.max_wait_time_ms , s.wait_time_ms , s.signal_wait_time_ms , s.wait_time_ms - s.signal_wait_time_ms  AS resource_wait_time_ms,
			100.0 * s.wait_time_ms  / SUM(s.wait_time_ms) OVER () AS percent_total_signal_wait,
			100.0 * s.wait_time_ms  / SUM(s.wait_time_ms) OVER () AS percent_total_signal_wait,
			100.0 * (s.wait_time_ms - s.signal_wait_time_ms) /  SUM(s.wait_time_ms) OVER () AS percent_total_resource_waits
